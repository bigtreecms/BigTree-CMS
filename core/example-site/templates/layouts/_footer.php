			...
			<nav class="navigation">
				...
				<?php
					foreach ($main_nav as $navItem) {
					    ...
					    ?>
				<div class="item">
					<a href="<?=$navItem['link']?>"<?php if ($active) {
    ?> class="active"<?php 
}
					    ?><?=targetBlank($navItem['link'])?>><?=$navItem['title']?></a>
					<?php
						if ($active && count($navItem['children'])) {
						    foreach ($navItem['children'] as $child) {
						        ?>
					<a href="<?=$child['link']?>" class="secondary<?php if (strpos($current_url, $child['link']) !== false) {
    ?> active<?php 
}
						        ?>"<?=targetBlank($child['link'])?>><?=$child['title']?></a>
					<?php
