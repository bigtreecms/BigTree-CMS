<?php
	...
	class PageService {
		public function list(Request $request) {
			...
			$where = "parent = ?";
			$args = [$parent];
			if (!$include_archived) $where .= " AND archived = ''";
		...
		public function update(Request $request) {
			...
			if (isset($d["nav_title"])) $update["nav_title"] = BigTree::safeEncode($d["nav_title"]);
			if (isset($d["title"])) $update["title"] = BigTree::safeEncode($d["title"]);
			if (isset($d["meta_keywords"])) $update["meta_keywords"] = (string)$d["meta_keywords"];
			if (isset($d["meta_description"])) $update["meta_description"] = (string)$d["meta_description"];
			if (isset($d["seo_invisible"])) $update["seo_invisible"] = !empty($d["seo_invisible"]) ? "on" : "";
			if (isset($d["in_nav"])) $update["in_nav"] = !empty($d["in_nav"]) ? "on" : "";
			if (isset($d["template"])) $update["template"] = (string)$d["template"];
			if (isset($d["external"])) $update["external"] = (string)$d["external"];
			if (isset($d["new_window"])) $update["new_window"] = !empty($d["new_window"]) ? "on" : "";
			if (isset($d["resources"])) $update["resources"] = json_encode($d["resources"]);
			if (isset($d["publish_at"])) $update["publish_at"] = $d["publish_at"];
			if (isset($d["expire_at"])) $update["expire_at"] = $d["expire_at"];
			if (isset($d["max_age"])) $update["max_age"] = (int)$d["max_age"];
