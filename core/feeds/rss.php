<rss version="0.91">
	<channel>
		<title><?php if ($feed['options']['feed_title']) {
    echo $feed['options']['feed_title'];
} else {
    echo $feed['name'];
} ?></title>
		<link><?php if ($feed['options']['feed_link']) {
    echo $feed['options']['feed_link'];
} else {
    ?><?=WWW_ROOT?>feeds/<?=$feed['route']?>/<?php 
} ?></link>
		...
		<item>
			...
			<description><![CDATA[<?=$blurb?><?php if ($blurb != $content) {
    ?><p><a href="<?=$link?>">Read More</a></p><?php 
}
			    ?>]]></description>
			<link><?=$link?></link>
		</item>
