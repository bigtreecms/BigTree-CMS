...
<?php
	...
	foreach ($items['resources'] as $resource) {
	    ?>
<a <?php if ($perm == 'n') {
    ?>href="#" class="file disabled"<?php 
} else {
    ?>href="<?=$resource['file']?>" class="file"<?php 
}
	    ?>><span class="icon_small icon_small_file_default icon_small_file_<?=$resource['type']?>"></span> <?=$resource['name']?></a>
<?php
