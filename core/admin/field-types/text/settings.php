...
?>
<fieldset>
	<label for="settings_field_sub_type"><?=Text::translate("Sub Type")?></label>
...
<fieldset id="settings_fieldset_max_length"<?php if (!empty($settings["sub_type"])) { ?> style="display: none;"<?php } ?>>
	<label for="settings_field_max_length"><?=Text::translate("Maximum Character Length <small>(leave empty or 0 for no max)</small>")?></label>
...
<fieldset id="settings_fieldset_seo_h1"<?php if (!empty($settings["sub_type"])) { ?> style="display: none;"<?php } ?>>
	...
	<label for="settings_field_seo_h1" class="for_checkbox"><?=Text::translate("Use For &lt;H1&gt; SEO Score <small>(only a single field can be used)</small>")?></label>
</fieldset>
<?php
	}
?>
