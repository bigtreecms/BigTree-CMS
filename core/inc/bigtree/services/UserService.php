<?php
	...
	class UserService {
		...
		public function update(Request $request) {
			...
			if (isset($d["name"])) $update["name"] = BigTree::safeEncode($d["name"]);
			if (isset($d["company"])) $update["company"] = BigTree::safeEncode($d["company"]);
			if (isset($d["timezone"])) $update["timezone"] = $d["timezone"];
			if (isset($d["daily_digest"])) $update["daily_digest"] = !empty($d["daily_digest"]) ? "on" : "";
			if (isset($d["alerts"]) && is_array($d["alerts"])) $update["alerts"] = $d["alerts"];
