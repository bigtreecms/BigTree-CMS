<?php
	...
	*/
	...
	class BigTreeAdminBase {
		...
		public static $ReservedTLRoutes = [
			"ajax",
			"css",
			"feeds",
			"js",
		...
		*/
		...
		public function __construct() {
			...
			} elseif (!empty($_COOKIE["bigtree_admin"]["email"]) && !empty($_COOKIE["bigtree_admin"]["login"])) {
				...
				if ($chain_entry && $chain_entry["csrf_token"]) {
					...
					if ($chain_entry["id"] == $session) {
						...
						if ($f) {
							...
							setcookie('bigtree_admin[login]', json_encode([$session, $chain]), strtotime("+1 month"), str_replace(DOMAIN, "", WWW_ROOT), "", false, true);
						}
			...
			foreach ($resources as $resource) {
				...
				if ($resource["id"] && $resource["id"] != "type") {
					...
					foreach ($resource as $k => $v) {
						if (!in_array($k, ["id", "title", "subtitle", "type", "settings", "options"])) {
							$field["settings"][$k] = $v;
						}
					}
...
');
	...
	*/');
		...
		*/
		...
		public function createModuleEmbedForm($module, $title, $table, $fields, $hooks = [], $default_position = "", $default_pending = "", $css = "", $redirect_url = "", $thank_you_message = "") {
			$clean_fields = [];
		...
		*/
		...
		public function createModuleForm($module, $title, $table, $fields, $hooks = [], $default_position = "", $return_view = false, $return_url = "", $tagging = "", $open_graph = "") {
			$clean_fields = [];
			...
			foreach ($fields as $key => $data) {
				...
				// Backwards compatibility with BigTree 4.1 package imports
				foreach ($data as $k => $v) {
					if (!in_array($k, ["title", "subtitle", "type", "options", "settings"])) {
						$field["settings"][$k] = $v;
					}
				}
		...
		*/
		...
		public function createPendingChange($table, $item_id, $changes, $mtm_changes = [], $tags_changes = [], $module = "") {
		...
		public function createTemplate($id, $name, $routed, $level, $module, $resources, $hooks = []) {
			...
			foreach ($resources as $resource) {
				if ($resource["id"]) {
					...
					// Backwards compatibility with BigTree 4.1 package imports
					foreach ($resource as $k => $v) {
						if (!in_array($k, ["id", "title", "subtitle", "type", "options", "settings"])) {
							$field["settings"][$k] = $v;
						}
					}
		...
		*/
		...
		public static function drawArrayLevel($keys, $level, $field = false) {
			...
			foreach ($level as $key => $value) {
				if (is_array($value)) {
					static::drawArrayLevel(array_merge($keys, [$key]), $value, $field);
				} else {
?>
...
<?php
		...
		*/
		...
		public function getAccessLevel($module, $item = [], $table = "", $user = false) {
			if (!$user) {
				$level = $this->Level;
		...
		*/
		...
		public function getCachedAccessLevel($module, $item = [], $table = "") {
			if ($this->Level > 0) {
				return "p";
			}
		...
		*/
		...
		public static function getFullNavigationPath($id, $path = []) {
		...
		*/
		...
		public function getPageAccessLevelByUser($page, $user) {
			...
			if (!is_numeric($page) && $page[0] == "p") {
				...
				// This change doesn't exist anymore - we can't check permissions.
				if (is_null($f)) {
					return $this->Level > 0 ? "p" : false;
				}

				$changes = json_decode($f["changes"], true);

				return !empty($changes["parent"]) ? $this->getPageAccessLevelByUser($changes["parent"], $user) : false;
			}
		...
		*/
		...
		public static function getContentsOfResourceFolder($folder, $sort = "date DESC") {
			...
			while ($f = sqlfetch($q)) {
				$f["file"] = BigTreeCMS::replaceRelativeRoots($f["file"]);
				$resources[] = $f;
			}
		...
		*/
		...
		public static function getResourceFolderBreadcrumb($folder, $crumb = []) {
			if (!is_array($folder)) {
		...
		*/
		...
		public static function growl($title, $message, $type = "success") {
			$_SESSION["bigtree_admin"]["growl"] = ["message" => $message, "title" => $title, "type" => $type];
		}
		...
		*/
		...
		public static function handleOpenGraph($table, $id, $data_source = null, $pending = false) {
			...
			if (!$og_image && !empty($data_source["image"])) {
		...
		*/
		...
		public function installExtension($manifest, $upgrade = false) {
			$bigtree["group_match"] = $bigtree["module_match"] = $bigtree["class_name_match"] = $bigtree["form_id_match"] = $bigtree["view_id_match"] = $bigtree["report_id_match"] = [];
		...
		*/
		...
		public function mergeTags($tag, $merge_tags) {
			...
			$this->updateTagReferenceCounts([$tag]);
			...
			while ($f = sqlfetch($q)) {
				if (isset($deleted_users[$f["user"]])) {
					...
					if (empty($users[$f["user"]])) {
						...
						$users[$f["user"]] = ["id" => $u["id"], "name" => $u["name"], "email" => $u["email"], "level" => $u["level"]];
					}
		...
		*/
		...
		public static function searchPages($query, $fields = ["nav_title"], $max = 10) {
			$results = [];
			$terms = explode(" ", $query);
			$qpart = ["archived != 'on'"];
		...
		*/
		...
		public function submitPageChange($page, $changes) {
			...
			if ($page[0] == "p") {
				...
				$pending = true;
				$existing_page = [];
				$existing_pending_change = ["id" => substr($page, 1)];
			} else {
		...
		*/
		...
		public static function unCache($page) {
			...
			@unlink(md5(json_encode(["bigtree_htaccess_url" => $url])).".page");
			@unlink(md5(json_encode(["bigtree_htaccess_url" => rtrim($url, "/")])).".page");
		}
		...
		*/
		...
		public function updateModuleEmbedForm($id, $title, $table, $fields, $hooks = [], $default_position = "", $default_pending = "", $css = "", $redirect_url = "", $thank_you_message = "") {
			$modules = BigTreeJSONDB::getAll("modules");
			$clean_fields = [];
		...
		*/
		...
		public function updateModuleForm($id, $title, $table, $fields, $hooks = [], $default_position = "", $return_view = false, $return_url = "", $tagging = "", $open_graph = "") {
			$clean_fields = [];
			$modules = BigTreeJSONDB::getAll("modules");
		...
		*/
		...
		public function updatePendingChange($id, $changes, $mtm_changes = [], $tags_changes = []) {
			$id = sqlescape($id);
			$changes = BigTree::json($changes, true);
			$mtm_changes = BigTree::json($mtm_changes, true);
			$tags_changes = BigTree::json($tags_changes, true);
		...
		*/
		...
		public static function updateTagReferenceCounts($tags = []) {
			if (!count($tags)) {
				$tags = [];
		...
		*/
		...
		public function verifyCSRFToken() {
			...
			$token = $_POST[$this->CSRFTokenField] ?? $_GET[$this->CSRFTokenField] ?? null;
			...
			if (strpos($clean_referer, $clean_domain) !== 0 || $token != $this->CSRFToken) {
				...
				while ($old_session = sqlfetch($q)) {
					$token = $_POST[$old_session["csrf_token_field"]] ?? $_GET[$old_session["csrf_token_field"]] ?? null;

					if ($token == $old_session["csrf_token"]) {
						return;
					}
				}
