	...
	class BigTreePaymentGateway {
		...
		*/
		...
		public function __construct($gateway_override = false) {
			...
			if ($s === false) {
				$s = ["service" => "", "settings" => []];
				BigTreeAdmin::updateInternalSettingValue("bigtree-internal-payment-gateway", $s, true);
			}
		...
		*/
		...
		public function saveSettings() {
			BigTreeAdmin::updateInternalSettingValue("bigtree-internal-payment-gateway", [
		...
		*/
		...
		protected function sendAuthorize($params) {
			...
			while ($count < 3) {
				$response = BigTree::cURL($this->PostURL, false, $curl_options);
