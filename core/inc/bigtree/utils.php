	...
	*/
	...
	class BigTree
	{
		...
		*/
		...
		public static function classAutoLoader($class)
		{
		    ...
		    if (isset($bigtree['other_classes'][$class])) {
		        include_once self::path($bigtree['other_classes'][$class]);
		...
		*/
		...
		public static function cURL($url, $post = false, $options = array(), $strict_security = false, $output_file = false)
		{
		    global $bigtree;
		        ...
		        if ($r != '.' && $r != '..' && $r != '.DS_Store' && $r != '__MACOSX') {
		            if ($include_git || ($r != '.git' && $r != '.gitignore')) {
		                ...
		                if (is_dir($path) && $recurse) {
		                    $contents = array_merge($contents, self::directoryContents($path, $recurse, $extension, $include_git));
		                }
		            }
		        }
		    }
		...
		*/
		...
		public function phpDateTojQuery($format)
		{
		    $new_format = '';
		...
		*/
		...
		public static function placeholderImage($width, $height, $bg_color = false, $text_color = false, $icon_path = false, $text_string = false)
		{
			...
			// Add text if provided or default to size
			    ...
			    $font = self::path('inc/lib/fonts/arial.ttf');
		...
		*/
		...
		public static function redirect($url = false, $codes = array('302'))
		{
		...
		*/
		...
		public static function sendEmail($to, $subject, $html, $text = '', $from = false, $return = false, $cc = false, $bcc = false, $headers = array())
		{
		...
		*/
		...
		public static function tableCompare($table_a, $table_b)
		{
			...
			$table_a_description = self::describeTable($table_a);
		    $table_a_columns = $table_a_description['columns'];
			// Get table B's description
			$table_b_description = self::describeTable($table_b);
		...
		*/
		...
		public static function unzip($file, $destination)
		{
			...
			if (!self::isDirectoryWritable($destination)) {
			    return false;
			}
			...
			if (class_exists('ZipArchive')) {
			    ...
			    for ($i = 0; $i < $z->numFiles; ++$i) {
			        ...
			        self::putFile($destination.$file['name'], $content);
			    }
			...
			// Fall back on PclZip if we don't have the "native" version.
			    ...
			    foreach ($archive as $item) {
			        ...
			        self::putFile($destination.$item['filename'], $item['content']);
			    }
