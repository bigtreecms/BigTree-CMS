	...
	<form method="post" action="<?=DEVELOPER_ROOT?>cloud-storage/google/activate/" class="module" enctype="multipart/form-data">
		<section>
			...
			<ol>
				...
				<li><?=Text::translate('Enter').' '.DEVELOPER_ROOT.'cloud-storage/google/return/ '.Text::translate('as an "Authorized Redirect URI" and choose "Web Application" for the Application Type.')?></li>
			...
			<fieldset>
				...
				<input type="text" name="project" value="<?=Text::htmlEncode($google->Project)?>" />
			</fieldset>
			<fieldset>
				<label><?=Text::translate("Client ID")?></label>
				<input type="text" name="key" value="<?=Text::htmlEncode($google->Key)?>" />
			</fieldset>
			<fieldset>
