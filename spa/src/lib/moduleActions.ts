 ...
 * within the user's level, and runnable in the SPA. Preserves the order the
 * actions endpoint returns (position DESC).
 *
 * Legacy custom-PHP actions (`render: "server"`) are omitted — classic admin
 * UI routing is gone; ModuleDispatcher shows a porting empty state if opened.
 */
