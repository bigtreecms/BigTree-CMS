/**
 * Gravatar URL builder keyed on the lowercased, trimmed email's MD5. Mirrors the
 * PHP admin's `BigTree::gravatar()` helper.
 *
 * Uses `d=404` so emails without a Gravatar return an HTTP error rather than a
 * generic silhouette — letting the caller (see `ui/Avatar`) fall back to its own
 * initials disc.
 */
export const gravatarUrl = (email: string, size: number): string => {
	const hash = md5Hex(email.trim().toLowerCase());

	return `https://www.gravatar.com/avatar/${hash}?s=${size * 2}&d=404&rating=pg`;
};
