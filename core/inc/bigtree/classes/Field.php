	...
	*/
	...
	class Field extends BaseObject {
		...
		*/
		...
		function __construct(array $field) {
			...
			$this->Output = false;
			$this->Settings = Link::decode(array_filter((array) ($field["options"] ?: [])));
		...
		*/
		...
		function drawArrayLevel(array $keys, array $level): void {
	...
	<?php
		...
		*/
		...
		function processImageUpload(): ?string {
			global $bigtree;
			...
			if ($error == 1 || $error == 2) {
				$bigtree["errors"][] = ["field" => $this->Title, "error" => "The file you uploaded ($name) was too large &mdash; <strong>Max file size: ".ini_get("upload_max_filesize")."</strong>"];
				
				return null;
			} elseif ($error == 3) {
				$bigtree["errors"][] = ["field" => $this->Title, "error" => "The file upload failed ($name)."];
			...
			if ((isset($this->Settings["min_height"]) && $iheight < $this->Settings["min_height"]) || (isset($this->Settings["min_width"]) && $iwidth < $this->Settings["min_width"])) {
				...
				$bigtree["errors"][] = ["field" => $this->Title, "error" => $error];
				$failed = true;
			}
			
			// If it's not a valid image, throw it out!
			if ($itype != IMAGETYPE_GIF && $itype != IMAGETYPE_JPEG && $itype != IMAGETYPE_PNG) {
				$bigtree["errors"][] = ["field" => $this->Title, "error" => "An invalid file was uploaded. Valid file types: JPG, GIF, PNG."];
				$failed = true;
			}
			
			// See if it's CMYK
			if ($channels == 4) {
				$bigtree["errors"][] = ["field" => $this->Title, "error" => "A CMYK encoded file was uploaded. Please upload an RBG image."];
				$failed = true;
			}
					...
					foreach ($this->Settings["crops"] as $crop) {
						if (!$failed && is_array($crop) && array_filter($crop)) {
							...
							if (!Image::getMemoryAvailability($temp_name, $crop["width"], $crop["height"])) {
								$bigtree["errors"][] = ["field" => $this->Title, "error" => "Image uploaded is too large for the server to manipulate. Please upload a smaller version of this image."];
								$failed = true;
							}
						}
					}
			...
			if (!$failed) {
				...
				$itype_exts = [IMAGETYPE_PNG => ".png", IMAGETYPE_JPEG => ".jpg", IMAGETYPE_GIF => ".gif"];
				...
				if (!$this->FileOutput) {
					if ($storage->DisabledFileError) {
						$bigtree["errors"][] = ["field" => $this->Title, "error" => "Could not upload file. The file extension is not allowed."];
					} else {
						$bigtree["errors"][] = ["field" => $this->Title, "error" => "Could not upload file. The destination is not writable."];
					}
		...
		*/
		...
		static function validationErrorMessage($data, string $type): string {
			$parts = explode(" ", $type);
			// Not required and it's blank
			$message = "This field ";
			$mparts = [];
