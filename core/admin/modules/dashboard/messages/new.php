...
<div class="container">
    ...
    ?> class="error_message">Errors found! Please fix the highlighted fields before submitting.</p>
			<fieldset id="send_to"<?php if ($error && !count($send_to)) {
    ?> class="form_error"<?php 
}
    ?>>
				<label class="required">Send To<?php if ($error && !count($send_to)) {
    ?><span class="form_error_reason">Required</span><?php 
}
    ?></label>
    ...
    ?>
			...
			<fieldset<?php if ($error && !$subject) {
    ?> class="form_error"<?php 
}
    ?>>
				<label class="required">Subject<?php if ($error && !$subject) {
    ?><span class="form_error_reason">Required</span><?php 
}
    ?></label>
				<input type="text" name="subject"  class="required" value="<?=$subject?>" />
			</fieldset>
			<fieldset<?php if ($error && !$message) {
    ?> class="form_error"<?php 
}
    ?>>
				<label class="required">Message<?php if ($error && !$message) {
    ?><span class="form_error_reason">Required</span><?php 
}
    ?></label>
