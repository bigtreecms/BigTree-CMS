...
<div class="contain">
	<div class="left date_pickers">
		<fieldset class="last">
			...
			<input type="text" class="date_picker" id="publish_at" name="publish_at" tabindex="3" value="<?php if ($bigtree['current_page']['publish_at']) {
    echo date('Y-m-d', strtotime($bigtree['current_page']['publish_at']));
} ?>" />
		...
		<fieldset class="right last">
			...
			<input type="text" class="date_picker" id="expire_at" name="expire_at" tabindex="4" value="<?php if ($bigtree['current_page']['expire_at']) {
    echo date('Y-m-d', strtotime($bigtree['current_page']['expire_at']));
} ?>" />
			<span class="icon_small icon_small_calendar date_picker_icon"></span>
		</fieldset>
	</div>
    ...
    ?>
				<option value="<?=$v?>"<?php if ($v == $bigtree['current_page']['max_age']) {
...
<div class="contain">
    ...
    ?>
		<input type="checkbox" name="in_nav" <?php if ($bigtree['current_page']['in_nav']) {
    ?>checked="checked" <?php 
}
    ?>class="checkbox" tabindex="7" />
    ...
    ?>disabled="disabled" class="checkbox" tabindex="7" />
	...
	<?php
		if (!$hide_template_section) {
		    ?>
	<fieldset class="float_margin">
		<input type="checkbox" name="redirect_lower" id="redirect_lower"<?php if ($bigtree['current_page']['template'] == '!') {
    ?> checked="checked"<?php 
}
		    ?> />
	...
	<?php
		...
		if ($admin->Level > 1) {
		    ?>
	<fieldset class="float_margin">
		<input type="checkbox" name="trunk" <?php if ($bigtree['current_page']['trunk']) {
    ?>checked="checked" <?php 
}
		    ?> tabindex="6" />
...
<div class="contain">
	<fieldset class="template last">
		...
		<select id="template_select" name="template"<?php if ($bigtree['current_page']['template'] == '!' || $bigtree['current_page']['external']) {
    ...
    ?>
				<option value="<?=$t['id']?>"<?php if ($t['id'] == $bigtree['current_page']['template']) {
    ?> selected="selected"<?php 
}
    ?>><?=$t['name']?></option>
				<?php 
	...
	<fieldset class="external last">
		...
		<input id="external_link" type="text" name="external" value="<?=$bigtree['current_page']['external']?>" id="external_link"<?php if ($bigtree['current_page']['template'] == '!') {
    ?> disabled="disabled"<?php 
}
    ?> />
	</fieldset>
	<fieldset class="checkbox_bump last">
		<input id="new_window" type="checkbox" name="new_window" value="Yes"<?php if ($bigtree['current_page']['new_window'] == 'Yes') {
    ?> checked="checked"<?php 
}
    ?><?php if ($bigtree['current_page']['template'] == '!') {
    ?> disabled="disabled"<?php 
}
    ?> />
