...
export const TabbedEditor = ({ tabs, value, onChange, className }: TabbedEditorProps) => {
	return (
		...
		>
			<Tabs.List className="flex items-center gap-1 border-b border-border bg-surface px-1">
				{tabs.map((tab) => (
					<Tabs.Trigger
						key={tab.value}
						value={tab.value}
						disabled={tab.disabled}
						className="-mb-px flex items-center gap-1.5 border-b-2 border-transparent px-3 py-2 text-[13px] font-medium text-text-3 transition-colors hover:text-text disabled:cursor-not-allowed disabled:opacity-40 data-[state=active]:border-accent data-[state=active]:text-accent"
					>
						{tab.icon}
						<span>{tab.label}</span>
					</Tabs.Trigger>
