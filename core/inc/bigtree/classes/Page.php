		...
		static function getPageDraft($id) {
			...
			if (is_array($tags_changes)) {
				foreach ($tags_changes as $tag) {
					$page->Tags[] = SQL::fetch("SELECT * FROM bigtree_tags WHERE id = ?", $tag);
				}
			}
			...
			// Protected vars
			$page->ChangesApplied = true;
			$page->UpdatedAt = $pending["date"];
			$page->LastEditedBy = $pending["user"] ?: $page->LastEditedBy;
