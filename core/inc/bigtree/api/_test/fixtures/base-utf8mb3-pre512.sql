-- FROZEN pre-512 schema — do not "keep this up to date" with core/setup/base.sql.
--
-- This is core/setup/base.sql exactly as it stood at revision 511, when every
-- core table was still CHARSET=utf8 (utf8mb3) and every varchar(1024) column was
-- indexed in full. It exists so CI can prove the UPGRADED path converges with the
-- fresh one: load this, run `php core/admin/migrate-run.php` (which applies
-- revision 512), and the schema + the API suite must come out the same as an
-- install built from today's base.sql (plans/utf8mb4-migration.md, V3).
--
-- Only touch this file if you need to test migrating from a DIFFERENT starting
-- point. A new revision after 512 does NOT belong here.
