	...
	<form method="post" action="<?=DEVELOPER_ROOT?>cloud-storage/set-default/">
		...
		<section>
			<div class="contain">
				...
				<a class="box_select last_row<?php if (!empty($cloud->Settings["rackspace"]["active"])) { ?> connected<?php } ?>" href="<?=DEVELOPER_ROOT?>cloud-storage/rackspace/">
					<span class="rackspace"></span>
					<p>Rackspace Cloud Files</p>
				</a>
				<a class="box_select last_row<?php if (!empty($cloud->Settings["google"]["active"])) { ?> connected<?php } ?>" href="<?=DEVELOPER_ROOT?>cloud-storage/google/">
			...
			<fieldset>
				...
				<select name="service">
					...
					<?php
						...
						if (!empty($cloud->Settings["google"]["active"])) {
					?>
