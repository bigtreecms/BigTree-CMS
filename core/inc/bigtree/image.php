	...
	class BigTreeImage {
		...
		public function __construct($file, $settings = []) {
			...
			if ($this->Type != IMAGETYPE_JPEG && $this->Type != IMAGETYPE_GIF && $this->Type != IMAGETYPE_PNG) {
				$this->Error = "This file has is an invalid image or is an unsupported image type.";
			...
			if (strpos($file, sys_get_temp_dir()) === 0 || strpos($file, ini_get("upload_tmp_dir")) === 0) {
		...
		*/
		...
		/*
			Function: getRequirementsError
				Returns an error message based on the passed in requirements
		
			Returns:
				An error string or null if no error was flagged.
		*/
		...
		public function getRequirementsError() {
			...
			if ($this->Type != IMAGETYPE_GIF && $this->Type != IMAGETYPE_JPEG && $this->Type != IMAGETYPE_PNG) {
				return "An invalid file was uploaded. Valid file types: JPG, GIF, PNG.";
			}
			
			// See if it's CMYK
			if ($this->ColorChannels == 4) {
				return "A CMYK encoded file was uploaded. Please upload an RBG image.";
			}
			
			$min_height = !empty($this->Settings["min_height"]) ? intval($this->Settings["min_height"]) : 0;
			$min_width = !empty($this->Settings["min_width"]) ? intval($this->Settings["min_width"]) : 0;
			
			// If the minimum height or width is not met, do NOT let the image through. Erase the change or update from the database.
			if (($min_height && $this->Height < $min_height) || ($min_width && $this->Width < $min_width)) {
		...
		*/
		...
		/*
			Function: processThumbnails
				Parses the image settings thumbs array and runs thumbnailing.
		*/
