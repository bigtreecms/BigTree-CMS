		...
		function drawXMLSitemap() {
			...
			while ($f = sqlfetch($q)) {
				if ($f["template"] || strpos($f["external"],$GLOBALS["domain"])) {	
					...
					$tf = sqlfetch(sqlquery("SELECT t.routed,m.class AS module_class FROM bigtree_templates AS t, bigtree_modules AS m WHERE t.id = '" . $f["template"] . "' AND (t.routed = '' OR m.id = t.module)"));
					if ($tf["routed"] == "on") {
