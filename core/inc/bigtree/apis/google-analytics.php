	...
	class BigTreeGoogleAnalyticsAPI extends BigTreeOAuthAPIBase {
		...
		*/
		...
		public function getAccounts($params = []) {
			...
			return new BigTreeGoogleResultSet($this, "getAccounts", [$params], $response, $results);
		}
		...
		*/
		...
		public function getProperties($account = "~all", $params = []) {
			...
			return new BigTreeGoogleResultSet($this, "getProperties", [$account, $params], $response, $results);
		}
		...
		*/
		...
		public function getProfiles($account = "~all", $property = "~all", $params = []) {
			...
			return new BigTreeGoogleResultSet($this, "getProfiles", [$account, $property, $params], $response, $results);
		}
		...
		*/
		...
		private function cacheParseLastData() {
			...
			if (!empty($d->visits)) {
				$time_on_site = round($d->timeOnSite ?? 0 / $d->visits, 2);
				...
				$bounce_rate = round($d->bounces ?? 0 / $d->visits * 100, 2);
			} else {
				$time = "00:00:00";
				$bounce_rate = 0;
			}
			...
			return [
				...
				"average_time" => $time,
				"average_time_seconds" => $time_on_site ?? 0,
			];
		}
