		...
		<div class="graph">
			<?php
			    ...
			    foreach ($visits as $date => $count) {
			        ...
			        ?>
			<section class="bar<?php if ($x == 14) {
    ?> last<?php 
} elseif ($x == 1) {
    ?> first<?php 
}
			        ?>" style="height: <?=$height?>px; margin-top: <?=(82 - $height)?>px;">
				<?=$count?>
			</section>
			<?php
			    ...
			    foreach ($visits as $date => $count) {
			        ++$x;
			        ?>
			<section class="date<?php if ($x == 14) {
    ?> last<?php 
} elseif ($x == 1) {
    ?> first<?php 
}
			        ?>"><?=date('n/j/y', strtotime($date))?></section>
			<?php
		...
		<section class="changes">
			<?php
				foreach ($change_modules as $m => $cm) {
				    ...
				    ?>
			<div>
				<a href="<?=ADMIN_ROOT?>dashboard/pending-changes/#<?=$m?>"><span class="icon_small icon_small_<?=$icon?>"></span> <?=$cm['count']?> change<?php if ($cm['count'] != 1) {
    ?>s<?php 
}
				    ?> for <?=$cm['title']?></a>
			</div>
		...
		<section class="changes">
			<?php
				foreach ($my_change_modules as $m => $cm) {
				    ...
				    ?>
			<div>
				<span class="icon_small icon_small_<?=$icon?>"></span> <?=$cm['count']?> change<?php if ($cm['count'] != 1) {
    ?>s<?php 
}
				    ?> for <?=$cm['title']?>
			</div>
