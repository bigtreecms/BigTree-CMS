	...
	*/
	...
	class AuthenticatedUser {
		...
		*/
		...
		function getAccessLevel($object, $entry = false, $table = "") {
		...
		*/
		...
		function getAccessibleModuleGroups(Module $module) {
		...
		*/
		...
		function getCachedAccessLevel(Module $module, $item = array(), $table = "") {
			$module_id = $module->ID;
			$permission = $this->getAccessLevel($module);

			// If group based permissions aren't on or we're a publisher of this module it's an easy solution… or if we're not even using the table.
			if (empty($item) || empty($module->GroupBasedPermissions["enabled"]) || $permission == "p" || $table != $module->GroupBasedPermissions["table"]) {
				return $permission;
			}
		...
		*/
		...
		function getGroupAccessLevel(Module $module, $group) {
			if ($this->Level > 0) {
				return "p";
			}
		...
		*/
		...
		function requirePublisher($object) {
			$access = $this->getAccessLevel($object);
