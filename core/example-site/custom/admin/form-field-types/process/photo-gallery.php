	...
	if (is_array($field["input"])) {
		foreach ($field["input"] as $photo_count => $data) {
			...
			// Otherwise, let's see if we have file information in the files array.
				...
				if (!$failed) {	
					$photo_gallery[] = array(
						"caption" => htmlspecialchars(htmlspecialchars_decode($data["caption"])), 
						"attribution" => htmlspecialchars(htmlspecialchars_decode($data["attribution"])), 
						"link" => htmlspecialchars(htmlspecialchars_decode($data["link"])), 
						"image" => $field["output"]
					);
				}
			}
