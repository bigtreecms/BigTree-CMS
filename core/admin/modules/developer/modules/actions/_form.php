...
<section>
    ...
    ?> selected="selected"<?php 
			...
			<option value="2"<?php if ($item['level'] == 2) {
    ?> selected="selected"<?php 
} ?>>Developer</option>
		</select>
	</fieldset>
	<div class="triplets">
		<fieldset>
			<label>Form</label>
			<select name="form"<?php if ($item['view'] || $item['report']) {
    ?> disabled="disabled"<?php 
} ?>>
				<option value="">&mdash;</option>
				<?php foreach ($forms as $form) {
    ?>
				<option value="<?=$form['id']?>"<?php if ($form['id'] == $item['form']) {
    ?> selected="selected"<?php 
}
    ?>><?=$form['title']?> (<?=$form['table']?>)</option>
				<?php 
} ?>
			</select>
		</fieldset>
		<fieldset>
			<label>View</label>
			<select name="view"<?php if ($item['form'] || $item['report']) {
    ?> disabled="disabled"<?php 
} ?>>
				<option value="">&mdash;</option>
				<?php foreach ($views as $view) {
    ?>
				<option value="<?=$view['id']?>"<?php if ($view['id'] == $item['view']) {
    ?> selected="selected"<?php 
}
    ?>><?=$view['title']?> (<?=$view['table']?>)</option>
				<?php 
} ?>
			</select>
		</fieldset>
		<fieldset>
			<label>Report</label>
			<select name="report"<?php if ($item['view'] || $item['form']) {
    ?> disabled="disabled"<?php 
} ?>>
				<option value="">&mdash;</option>
				<?php foreach ($reports as $report) {
    ?>
				<option value="<?=$report['id']?>"<?php if ($report['id'] == $item['report']) {
    ?> selected="selected"<?php 
}
    ?>><?=$report['title']?> (<?=$report['table']?>)</option>
				<?php 
} ?>
			</select>
		</fieldset>
    ...
    ?>
			<li>
				<a href="#<?=$class?>"<?php if ($class == $item['class']) {
    ?> class="active"<?php 
}
    ?>><span class="icon_small icon_small_<?=$class?>"></span></a>
			</li>
			<?php 
} ?>
		</ul>
	...
	<fieldset>
		...
		<input type="checkbox" name="in_nav" <?php if ($item['in_nav']) {
    ?>checked="checked" <?php 
} ?>/>
	</fieldset>
</section>
