...
<div class="container">
	<header>
		<div class="sticky_controls">
			<div class="shadow">
				<nav class="left">
					<a href="#properties_tab"<?php if ($bigtree['form_action'] == 'create') {
    ?> class="active"<?php 
} ?>>Properties</a>
					<a href="#content_tab"<?php if ($bigtree['form_action'] == 'update') {
    ?> class="active"<?php 
} ?>>Content</a>
	...
	<form method="post" class="module" action="<?=ADMIN_ROOT?>pages/<?=$bigtree['form_action']?>/" enctype="multipart/form-data" id="page_form">
		...
		<input type="hidden" name="<?php if ($bigtree['form_action'] == 'create') {
    ?>parent<?php 
} else {
    ?>page<?php 
} ?>" value="<?=$bigtree['current_page']['id']?>" />
		
		<section id="properties_tab"<?php if ($bigtree['form_action'] == 'update') {
    ?> style="display: none;"<?php 
} ?>>
			<?php include BigTree::path('admin/modules/pages/tabs/properties.php') ?>
		</section>
		<section id="content_tab"<?php if ($bigtree['form_action'] == 'create') {
    ?> style="display: none;"<?php 
} ?>>
		...
		<footer>
			...
			<?php
				if ($bigtree['form_action'] == 'create') {
				    ?>
			<input type="submit" name="ptype" value="Create" <?php if ($bigtree['access_level'] != 'p') {
    ?>class="blue" <?php 
}
				    ?>/>
			<?php
			...
			<input type="submit" name="ptype" value="Save"<?php if ($bigtree['access_level'] != 'p') {
    ?> class="blue"<?php 
}
				    ?> />
			<?php
