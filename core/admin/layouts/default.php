...
<div id="page">
	<?php
		if ($bigtree['page']['title'] && !defined('BIGTREE_404')) {
		    ?>
	<h1>
		<span class="page_icon <?=$bigtree['page']['icon']?>"><?php if ($bigtree['page']['icon'] == 'gravatar') {
    ?><img src="<?=BigTree::gravatar($bigtree['gravatar'])?>" alt="" /><?php 
}
		    ?></span>
		<?php
			$x = 0;
		    foreach ($breadcrumb as $item) {
		        ...
		        ?>
		<a href="<?=ADMIN_ROOT.$item['link']?>/" class="<?php if ($x == 1) {
    ?> first<?php 
}
		        if ($x == count($breadcrumb)) {
		            ?> last<?php 
		        }
		        ?>"><?=BigTree::safeEncode($item['title'])?></a>
		<?php
				if ($x != count($breadcrumb)) {
				    ?>
		...
		<?php
			...
			foreach ($bigtree['page']['navigation'] as $item) {
			    if (!$item['hidden'] && (!$item['level'] || $item['level'] <= $admin->Level)) {
			        ...
			        ?>
		<a href="<?=ADMIN_ROOT.$item['link']?>/<?=htmlspecialchars(rtrim($get_string, '&'))?>"<?php if ($active_item == $item) {
    ?> class="active"<?php 
}
			        ?>><span class="icon_small icon_small_<?=($item['nav_icon'] ? $item['nav_icon'] : $item['icon'])?>"></span><?=$item['title']?></a>
		<?php

			    }
			}
		    ?>
		<menu<?php if (!count($bigtree['subnav_extras'])) {
