	...
	class Image
	{
		...
		public function __construct(string $file, array $settings = [], bool $ignore_minimums = false)
		{
			...
			if ($this->Type != IMAGETYPE_JPEG && $this->Type != IMAGETYPE_GIF && $this->Type != IMAGETYPE_PNG) {
				$this->Error = Text::translate("The file is an invalid image or is an unsupported image type.");
			...
			if (!$ignore_minimums && ($this->Height < $this->MinHeight || $this->Width < $this->MinWidth)) {
				...
				if ($this->MinHeight && $this->MinWidth) {
					$error .= ":width: x :height: pixels.";
				} elseif ($this->MinHeight) {
					$error .= ":height: pixels tall.";
				} elseif ($this->MinWidth) {
					$error .= ":width: pixels wide.";
				}
				
				$this->Error = Text::translate($error, false, [":height:" => $this->MinHeight, ":width:" => $this->MinWidth]);
			...
			if ($this->ColorChannels == 4 && $this->Type == IMAGETYPE_JPEG) {
				$this->Error = Text::translate("A CMYK encoded file was uploaded. Please upload an RBG image.");
		...
		*/
		...
		public function destroy(): void
		{
			...
			$this->Error = Text::translate("The file has been deleted.");
			$this->Width = 0;
			$this->Height = 0;
			$this->Type = 0;
		}
		...
		*/
		...
		{
			if (!file_exists(SITE_ROOT."files/temporary/")) {
				mkdir(SITE_ROOT."files/temporary/");
				FileSystem::setDirectoryPermissions(SITE_ROOT."files/temporary/");
			}
			
			$temp_file = SITE_ROOT."files/temporary/".uniqid("temp-").$this->Extensions[$this->Type];
		...
		*/
		...
		public static function processCrop(string $crop_key, int $index, int $x, int $y, int $width, int $height): void
		{
			$storage = new Storage;
			...
			$temp_image = new Image($temp_crop);
		...
		public function replace(string $name, bool $force_local = false): ?string
		{
			...
			if (!$path) {
				if ($this->Storage->DisabledFileError) {
					$this->Error = Text::translate("Could not upload file. The file extension is not allowed.");
				} else {
					$this->Error = Text::translate("Could not upload file. The destination is not writable.");
				}
