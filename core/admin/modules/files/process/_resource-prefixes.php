<?php
	...
	foreach ($image->Settings["crops"] as $crop_index => $crop) {
		...
		if (is_array($crop["thumbs"])) {
			foreach ($crop["thumbs"] as $thumb) {
				if ($thumb["prefix"]) {
					$crop_prefixes[$thumb["prefix"]] = $image->getThumbnailSize(intval($thumb["width"]), intval($thumb["height"]),
																				intval($crop["width"]), intval($crop["height"]));
				}
			}
		}
	...
	foreach ($image->Settings["thumbs"] as $thumb) {
		if ($thumb["prefix"]) {
			$thumb_prefixes[$thumb["prefix"]] = $image->getThumbnailSize(intval($thumb["width"]), intval($thumb["height"]));
		}
	}
