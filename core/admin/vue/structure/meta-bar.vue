		...
		<div v-for="item in items" class="meta_bar_detail" :class='{ "js-tooltip": item.tooltip }'
			...
			<template v-if="item.type == 'visual'">
				<span class="meta_bar_value meta_bar_value_graph">
					<span class="meta_bar_visual"
						  :class='{ "meta_bar_visual_bad": item.value < 30, "meta_bar_visual_good": item.value > 30 && item.value <= 65, "meta_bar_visual_great": item.value > 65 }'
						  :style="'width: ' + (item.value >= 5 ? item.value : 5) + '%;'">{{ item.value }}</span>
				</span>
			</template>
			<template v-else>
