...
<?php
	// Draw the list.
	    ...
	    ?>
<select<?php if (count($class)) {
    ?> class="<?=implode(' ', $class)?>"<?php 
}
	    ?> name="<?=$field['key']?>" tabindex="<?=$field['tabindex']?>" id="<?=$field['id']?>">
    ...
    ?>
	<option value="<?=BigTree::safeEncode($option['value'])?>"<?php if ($field['value'] == $option['value']) {
    ?> selected="selected"<?php 
}
    ?><?php if ($option['access_level']) {
    ?> data-access-level="<?=$option['access_level']?>"<?php 
}
    ?>><?=BigTree::safeEncode(BigTree::trimLength(strip_tags($option['description']), 100))?></option>
	<?php 
}
	    ?>
</select>
