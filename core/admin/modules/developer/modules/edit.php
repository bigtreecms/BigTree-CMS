...
<div class="container">
    ...
    ?>
					<option value="<?=$group['id']?>"<?php if ($group['id'] == $module['group']) {
    ...
    ?>
		        	<li>
		        		<a href="#<?=$class?>"<?php if ($class == $module['icon']) {
    ?> class="active"<?php 
}
    ?>><span class="icon_small icon_small_<?=$class?>"></span></a>
			...
			<fieldset>
				<input type="checkbox" name="gbp[enabled]" id="gbp_on" <?php if (isset($gbp['enabled']) && $gbp['enabled']) {
    ?>checked="checked" <?php 
} ?>/>
...
<div class="table">
	...
	<?php
		...
		if (count($actions_not_in_nav)) {
		    ?>
	<ul<?php if (count($actions_in_nav)) {
    ?> class="secondary"<?php 
}
		    ?>>
