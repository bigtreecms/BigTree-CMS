<?php
	...
	class TagService {
		...
		public function get(Request $request) {
			...
			if (!$row) throw new NotFoundException("Tag $id not found", "resource_not_found", 404);
			return Response::ok($this->present($row));
		}

		public function create(Request $request) {
			$tag = $this->normalize((string)$request->body["tag"]);
			if ($tag === "") throw new BadRequestException("Empty tag after normalization", "empty_tag", 400);
		...
		public function merge(Request $request) {
			...
			if (!$into || !$from) throw new BadRequestException("into and from required", "missing_fields", 400);
			if (in_array($into, $from, true)) throw new BadRequestException("into cannot be in from list", "invalid_merge", 400);
