<?php
	...
	if (defined('BIGTREE_CALLOUT_RESOURCES')) {
	    ...
	    ?>
<input type="hidden" name="<?=$field['key']?>" value="<?php if ($field['value']) {
    echo date('h:i a', strtotime($field['value']));
}
	    ?>" />
...
<?php	
	    ...
	    ?>
<input type="text" tabindex="<?=$field['tabindex']?>" name="<?=$field['key']?>" value="<?php if ($field['value']) {
    echo date('h:i a', strtotime($field['value']));
}
	    ?>" autocomplete="off" id="<?=$field['id']?>" class="time_picker<?php if ($field['required']) {
    ?> required<?php 
}
	    ?>" />
