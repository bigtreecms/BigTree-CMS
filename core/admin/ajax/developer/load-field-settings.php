...
<fieldset>
	...
	<select id="settings_field_validation" name="validation">
		<option></option>
		<?php foreach ($validation_options as $key => $value) { ?>
		<option value="<?=$key?>"<?php if ($key == $validation) { ?> selected="selected"<?php } ?>><?=$value?></option>
		<?php } ?>
	</select>
</fieldset>
<?php
...
<?php
	...
	if (strpos($field_type, "*") === false) {
		$path = Router::getIncludePath("admin/field-types/$field_type/settings.php");
	} else {
...
?>
<p><?=Text::translate("This field type does not have any settings.")?></p>
<?php
	}
?>
<script>	
