<?php
	...
	class AIChatService {
		...
		public function systemPrompt($user): string {
			...
			$lines[] = "- A value that came back truncated is not a value you can edit. Reads cap long fields and say which ones they cut (content_truncated on get_page, fields_truncated on get_module_entry and list_module_entries) — and the write tools replace a field's whole value, so writing a truncated value back would discard everything past the cut. If you need to change a field the read marked truncated, say so and ask the user to edit it in the admin.";
